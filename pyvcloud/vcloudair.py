...
class VCA(object):
    ...
    def _create_cloneVAppParams(self, vapp_href, name, deploy, power, source_delete):
    ...
    def _create_composeVAppParams(self, name, network_name, network_href, fence_mode, vm_specs,
            power, deploy):
        ...
        if network_href:
            ...
            networkConfigSection = vcloudType.NetworkConfigSectionType(NetworkConfig=[networkConfig])
            networkConfigSection.original_tagname_ = "NetworkConfigSection"
            networkConfigSection.set_Info(vAppType.cimString(valueOf_="Network"))

            instantiationParams.add_Section(networkConfigSection)

        composeParams = vcloudType.ComposeVAppParamsType(InstantiationParams=instantiationParams)
    ...
    def compose_vapp(self, vdc_name, vapp_name, network_name=None, network_mode='bridged',
            deploy='false', poweron='false', vm_specs=None):
        """
        ...
        :param vm_specs: (list): A list of dicts with keys 'vm_href', 'name*', 'cpus*', 'memory*'. *Optional.
        ...
        **service type:**. ondemand, subscription, vcd
        ...
        network_href = None
        if network_name:
            network_href = self.get_network(vdc_name, network_name).get_href()

        compose_params = self._create_composeVAppParams(vapp_name, network_name, network_href,
                network_mode, vm_specs, deploy=deploy, power=poweron)
    ...
    def clone_vapp(self, source_vdc_name, source_vapp_name, dest_vdc_name, dest_vapp_name,
            source_delete=False, deploy=False, poweron=False):
        """
        ...
        **service type:**. ondemand, subscription, vcd
        ...
        refs = filter(lambda ref: ref.name == source_vapp_name and ref.type_ ==
                'application/vnd.vmware.vcloud.vApp+xml', source_vdc.ResourceEntities.ResourceEntity)
        if len(refs) == 1:
            source_vapp_href = refs[0].href
        else:
            return False

        clone_params = self._create_cloneVAppParams(source_vapp_href, dest_vapp_name, deploy,
                poweron, source_delete)

        output = StringIO()
        clone_params.export(output,
            0,
    ...
    def create_vapp(self, vdc_name, vapp_name, template_name, catalog_name,
        ...
        **service type:**. ondemand, subscription, vcd
        ...
        vm_href = self.get_template_href_from_catalog(catalog_name, template_name)

        return self.compose_vapp(vdc_name, vapp_name, network_name, network_mode, deploy, poweron,
                                 vm_specs=[{'vm_href': vm_href, 'name': vm_name, 'cpus': vm_cpus,
                                            'memory': vm_memory}])
